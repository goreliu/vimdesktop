vimconfig:
	...
	;vim.SetAction("<vc_config>","打开VimDesktop配置界面")
	;vim.SetMode("normal")
	;vim.map("<w-tab>gp","<vc_plugin>")
	;vim.map("<w-tab>gk","<vc_keymap>")
	;vim.map("<f2>","<vc_keymap>")
	;vim.map("<f1>","<vc_config>")
return
/*
<vc_config>:
